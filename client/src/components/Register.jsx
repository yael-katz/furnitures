                ...
                content={({ hide }) => (
                    <div className="flex flex-column px-8 py-5 gap-4" style={{ borderRadius: '12px', backgroundImage: 'radial-gradient(circle at left top, var(--primary-400), var(--primary-700))' }}>
                        ...
                        <div className="inline-flex flex-column gap-2">
                            ...
                            onBlur={(e)=>{setFormData({userName:e.target.value,password:formData.password,name:formData.name,email:formData.email,phone:formData.phone})}}></InputText>
                        </div>
                        <div className="inline-flex flex-column gap-2">
                            <label htmlFor="username" className="text-primary-50 font-semibold">
                                password
                            ...
                            onBlur={(e)=>{setFormData({userName:formData.userName,password:e.target.value,name:formData.name,email:formData.email,phone:formData.phone})}}></InputText>
                        </div>
                        <div className="inline-flex flex-column gap-2">
                            <label htmlFor="name" className="text-primary-50 font-semibold">
                            name
                            </label>
                            <InputText id="name" label="name" className="bg-white-alpha-20 border-none p-3 text-primary-50"
                            onBlur={(e)=>{setFormData({userName:formData.userName,password:formData.password,name:e.target.value,email:formData.email,phone:formData.phone})}}></InputText>
                        </div>
                        <div className="inline-flex flex-column gap-2">
                            <label htmlFor="email" className="text-primary-50 font-semibold">
                            email
                            </label>
                            <InputText id="email" label="email" className="bg-white-alpha-20 border-none p-3 text-primary-50"
                            onBlur={(e)=>{setFormData({userName:formData.userName,password:formData.password,name:formData.name,email:e.target.value,phone:formData.phone})}}></InputText>
                        </div>
                        <div className="inline-flex flex-column gap-2">
                            <label htmlFor="phone" className="text-primary-50 font-semibold">
                            phone
                            </label>
                            <InputText id="phone" label="phone" className="bg-white-alpha-20 border-none p-3 text-primary-50"
                            onBlur={(e)=>{setFormData({userName:formData.userName,password:formData.password,name:formData.name,email:formData.email,phone:e.target.value})}}></InputText>
                        </div>
                        <div className="flex align-items-center gap-2">
                            <Button label="Sign-In" onClick={handleSubmit} text className="p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10"></Button>
